...
class NotificationTest {
	...
	public void testNotificationBuilder() throws GeneralSecurityException {
		final Notification notification = Notification.builder().endpoint(endpoint).userPublicKey(publicKey)
				.payload(new byte[16]).ttl((int) Duration.ofDays(15).getSeconds()).build();
	...
	public void testDefaultTtl() throws GeneralSecurityException {
		final Notification notification = Notification.builder().userPublicKey(publicKey).payload(new byte[16]).build();
