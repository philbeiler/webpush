...
public class SeleniumTests {
	...
	public Stream<DynamicTest> dynamicTests() throws IOException {
		...
		return getConfigurations().map(configuration -> {
			final BrowserTest browserTest = new BrowserTest(testingService, configuration, testSuiteId);
	...
	protected Stream<Configuration> getConfigurations() {
		final String PUBLIC_KEY_NO_PADDING = Base64.getUrlEncoder().withoutPadding()
				.encodeToString(Base64.getUrlDecoder().decode(PUBLIC_KEY));

		return Stream.of(new Configuration("chrome", "stable", null, GCM_SENDER_ID),
